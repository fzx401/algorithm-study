...
class Primary_Problem:  # 原问题
    ...
    def relaxed_object(self, multiplicator):  # 输入乘子，输出松弛之后，目标函数中变量的系数
        ...
        for i in range(0, self.variables_n):
            self.relaxed_object_coeff.append(
                {
                    "quadratic": self.object_coeff[i],
                    "linear": sum(multiplicator * self.constraint_coeff[:, i]),
                }
            )
        self.relaxed_object_constant = sum(
            multiplicator * self.object_constant
        )  # 松弛之后，目标函数中的常数项
    ...
    def solve_subproblem(self):  # 求解原问题对应的子问题，子问题数量等于变量个数
        ...
        x = m.addVars(self.variables_n, lb=0, vtype=GRB.CONTINUOUS, name="x")
        m.update()
        quad_expr = QuadExpr()
        quadratic = [
            self.relaxed_object_coeff[i]["quadratic"]
            for i in range(len(self.relaxed_object_coeff))
        ]
        linear = [
            self.relaxed_object_coeff[i]["linear"]
            for i in range(len(self.relaxed_object_coeff))
        ]
        for q in range(len(quadratic)):
            quad_expr.add(x[q] * x[q], quadratic[q])
        quad_expr.add(x.prod(linear), 1)
        quad_expr.add(self.relaxed_object_constant, 1)
        m.setObjective(quad_expr)
        m.optimize()
        vars = m.getVars()
        for i in range(0, self.variables_n):
            result = vars[i].getAttr("X")
            # result = (
            #     -1
            #     * self.relaxed_object_coeff[i]["linear"]
            #     / (2 * self.relaxed_object_coeff[i]["quadratic"])
            # )  # 让导数等于0来求解未知量的求解结果
            self.results[i]
    ...
    def upper_bound(self):  # 采用启发式算法，调整求解子问题所得到的解，使其可行，得到原问题解的上界
        for i in range(0, self.constraints_n):  # 第i个被松弛掉的约束
            ...
            while (
                sum(self.results * self.constraint_coeff[i, :])
                + self.object_constant[i]
                > 0
            ):  # 当被松弛的约束不满足要求时
                modified_index = result_lst.index(min(result_lst))  # 启发式算法，从值最小的开始调整
                modified_coeff = self.constraint_coeff[
                    i, modified_index
                ]  # 需要被调整的未知量在约束中对应的系数
                if (
                    self.results[modified_index] - modified_coeff / abs(modified_coeff)
                    >= 0
                ):  # 保证在调整的时候满足非负约束
                    self.results[modified_index] = self.results[
                        modified_index
                    ] - modified_coeff / abs(
                        modified_coeff
                    )  # 若是正系数，则进行-1操作，若负系数则+1操作，使得约束满足
                    result_lst[modified_index] = result_lst[
                        modified_index
                    ] - modified_coeff / abs(modified_coeff)
                else:
        ...
        self.upper_bound = sum(
            self.modified_results * self.modified_results * self.object_coeff
        )
...
class Dual_Problem:  # 对偶问题
    def __init__(
        self, object_coeff, constraint_coeff, subproblem_outcome, multiplicator
    ):  # 输入目标函数的系数，子问题的约束（被松弛的）,计算结果,乘子
    ...
    def relaxed_object(self):
        ...
        for i in range(0, self.variables_n):
            self.relaxed_object_coeff.append(
                {
                    "quadratic": self.object_coeff[i],
                    "linear": sum(self.multiplicator * self.constraint_coeff[:, i]),
                }
            )
        self.relaxed_object_constant = sum(
            self.multiplicator * self.object_constant
        )  # 松弛之后，目标函数中的常数项
    ...
    def lower_bound(self):
        ...
        for i in range(0, self.constraints_n):
            linear_results_lst.append(
                sum(self.multiplicator[i] * self.constraint_coeff[i, :] * self.results)
            )
        self.lower_bound = (
            self.constant + sum(self.quadratic_results) + sum(linear_results_lst)
        )
    ...
    def subgradient(self):  # 输出乘子对应的梯度
        ...
        for i in range(0, self.constraints_n):
            self.subgradient[i] = (
                np.dot(self.results, self.constraint_coeff[i, :])
                + self.object_constant[i]
            )
    ...
    def update_stepsize(self):  # 按照公式输出新的步长
        ...
        self.stepsize = (417 - self.function_value) / np.linalg.norm(
            self.subgradient
        ) ** 2
    ...
    def update_multiplier(self):  # 更新乘子
        for i in range(0, self.constraints_n):
            self.multiplicator[i] = (
                self.multiplicator[i] + self.subgradient[i] * self.stepsize
            )
...
if __name__ == "__main__":
    ...
    subproblem = Primary_Problem(
        object_coeff, constraint_coeff, object_constant
    )  # 实例化原问题
    ...
    dualproblem = Dual_Problem(
        object_coeff, constraint_coeff, subproblem_outcome, multiplicator
    )  # 实例化对偶问题
